"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var React = require("react");
var base64X = "base64,";
var base64Y = "base64,";
var SpacingX = (function (_super) {
    __extends(SpacingX, _super);
    function SpacingX() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    SpacingX.prototype.render = function () {
        return (React.createElement("img", { className: this.props.className, src: "data:image/png;" + base64X }));
    };
    return SpacingX;
}(React.Component));
exports.SpacingX = SpacingX;
var SpacingY = (function (_super) {
    __extends(SpacingY, _super);
    function SpacingY() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    SpacingY.prototype.render = function () {
        return (React.createElement("img", { className: this.props.className, style: { minWidth: "12px" }, src: "data:image/png;" + base64Y }));
    };
    return SpacingY;
}(React.Component));
exports.SpacingY = SpacingY;
//# sourceMappingURL=SpacingIcons.js.map