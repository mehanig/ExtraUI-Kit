...
var RadioButtonList = (function (_super) {
    ...
    RadioButtonList.prototype.render = function () {
        ...
        var liElements = this.state.values.map(function (itemValue, index) {
            return (<li key={itemValue + index} style={stylesLiArr}>
          <input checked={_this.state.selectedOption === itemValue} onChange={_this.handleOptionChange} style={cssInputArr} type="radio" name={_this.props.name} value={itemValue}/>
          <span style={[css.Text]} value={itemValue} onClick={_this.handleLiClick}>
            {_this.state.textValues[index]}
          </span>
        </li>);
        });
        return (<div>
